...
The input is marked with `filter-query`. The handler goes up to the space and reads that element, so it never assumes it was called from the input itself. The trigger can sit on the input, on a button next to it, or anywhere inside the space.
